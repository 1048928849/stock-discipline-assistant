...
- “交易计划生成器”：输入股票与账户后，确定性识别平台、放量突破、缩量回踩和再次转强，输出 `READY / WAIT / NO_TRADE / INSUFFICIENT_DATA`、12 道闸门、条件式买入区、硬止损、风险收益、确认加仓和退出纪律；
- 仓位同时受风险预算、可用资金、单股上限、账户总仓位和行业集中度约束，最终向下取 100 股整手；已有持仓会检查浮盈确认加仓、硬止损和第一减仓触发；
- 动态资金成本锚同时计算20/60/120日成交量加权成本、成交密集峰与平台结构锚，形成带来源和置信度的支撑/压力区及极限止损候选；候选在通过样本外验证前不会替换冻结的正式止损；
- 事件成本锚会从异常涨跌和异常成交中选择候选事件日；数据存在时计算开盘至10:00、10:30、上午、下午和全日VWAP，并与大宗交易等机构价格证据进行跨事件聚类。缺少分钟或机构数据时明确降级，不用日线数据伪造分时成本；
- 账户风险控制汇总全部持仓相对硬止损的开放风险，记录账户权益峰值和回撤；缺失持仓止损、组合风险超限或账户回撤触及阈值时禁止新增风险，并展示跳空压力损失；
- 建仓和卖出采用 `WATCH / READY / TRIAL / CONFIRMED / HOLD / REDUCE / EXIT / CLOSED` 统一生命周期；亏损加仓、结构破坏后加仓、非法重开均会被领域状态机拒绝；
- 策略验证采用严格时间顺序、walk-forward切分、手续费和滑点，检查期望值、利润因子、最大回撤、尾部损失与市场环境样本；没有至少30笔且通过的验证记录，策略版本不能从影子阶段激活；
- 计划预览不自动保存；用户确认后冻结行情、账户、规则、证据和参数快照。重新计算创建新版本，可查询历史并比较差异；
...
- 行情写入前执行完整性检查，失败时可明确回退最近成功数据并标记 `stale_fallback`；
